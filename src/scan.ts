...
import { discoverClaudeCodeSessions, parseClaudeCodeSession } from './adapters/claude-code.js';
...
/** Scan standalone Claude Code sessions (~/.claude/projects). */
export async function scanClaudeCode(opts: { dir?: string; quiet?: boolean } = {}): Promise<ScanResult> {
  const res: ScanResult = { scanned: 0, added: 0, updated: 0, skipped: 0, failed: [], zstSkipped: 0 };
  const idx = loadIndex();
  const known = new Map(Object.values(idx.sessions).map((e) => [e.sourcePath, e]));

  for (const d of discoverClaudeCodeSessions(opts.dir)) {
    res.scanned++;
    let st: fs.Stats;
    try {
      st = fs.statSync(d.transcript);
    ...
    try {
      const session = await parseClaudeCodeSession(d);
      saveSession(session);
      idx.sessions[session.meta.id] = entryFromMeta(session.meta, st.mtimeMs, st.size);
      prev ? res.updated++ : res.added++;
      if (!opts.quiet) {
...
export async function scanAll(opts: { quiet?: boolean } = {}): Promise<Record<string, ScanResult>> {
  return {
    ...
    'claude-code': await scanClaudeCode(opts),
  };
}
